...
class Transformer:
    ...
    def __init__(self, modelinfos, tensor_parallel=8):
        ...
        self.use_lora = modelinfos['use_lora']
        self.lora_rank = modelinfos['lora_rank']
    ...
    def build(self, batch, lin, lout, attn_on_hetero=False):
        ...
        if self.use_lora:
            self.sum_decoder.append(
                Layer('sum', 'lora_shrink_q', LayerType.FC, True, self.dtype, 
                      lin, int(self.lora_rank / self.tp), self.hdim, batch))
            self.sum_decoder.append(
                Layer('sum', 'lora_shrink_k', LayerType.FC, True, self.dtype, 
                      lin, int(self.lora_rank / self.tp), self.hdim, batch))
            self.sum_decoder.append(
                Layer('sum', 'lora_shrink_v', LayerType.FC, True, self.dtype, 
                      lin, int(self.lora_rank / self.tp), self.hdim, batch))
            self.sum_decoder.append(
        ...
        if 'LLAMA' in self.name:
            self.sum_decoder.append(
                Layer('sum', 'ff1_2', LayerType.FC, True, self.dtype, batch * lin,
                      2 * self.ff_scale * int(self.hdim / self.tp), self.hdim, 1))
            if self.use_lora:
                self.sum_decoder.append(
                    Layer('sum', 'lora_shrink_ffn1', LayerType.FC, True, self.dtype, 
                        lin, int(self.lora_rank / self.tp), self.hdim, batch))
                self.sum_decoder.append(
            ...
            if self.use_lora:
                self.sum_decoder.append(
                    Layer('sum', 'lora_shrink_ff1', LayerType.FC, True, self.dtype, 
                        lin, int(self.lora_rank / self.tp), self.hdim, batch))
                self.sum_decoder.append(
            ...
            self.sum_decoder.append(
                Layer('sum', 'ff2', LayerType.FC, True, self.dtype, 
                      batch * lin, self.hdim, self.ff_scale * int(self.hdim / self.tp), 1))
            if self.use_lora:
                self.sum_decoder.append(
        ...
        for stage in range(1, lout, 1):
            ...
            if self.use_lora:
                decoder.append(
                    Layer('gen', 'lora_shrink_q', LayerType.FC, True, self.dtype, 
                        1, int(self.lora_rank / self.tp), self.hdim, batch))
                decoder.append(
                    Layer('gen', 'lora_shrink_k', LayerType.FC, True, self.dtype, 
                        1, int(self.lora_rank / self.tp), self.hdim, batch))
                decoder.append(
                    Layer('gen', 'lora_shrink_v', LayerType.FC, True, self.dtype, 
                        1, int(self.lora_rank / self.tp), self.hdim, batch))
                decoder.append(
            ...
            if self.use_lora:
                decoder.append(
                    Layer('gen', 'lora_o_shrink_proj', LayerType.FC, True, self.dtype, 
                        1, int(self.lora_rank / self.tp), self.hdim, batch))
                decoder.append(
                    Layer('gen', 'lora_o_expand_proj', LayerType.FC, True, self.dtype, 
                        1, self.hdim, int(self.lora_rank / self.tp), batch))
            ...
            if 'LLAMA' in self.name:
                decoder.append(
                    Layer('gen', 'ff1_2', LayerType.FC, True, self.dtype, 
                          batch, 2 * self.ff_scale * int(self.hdim / self.tp), self.hdim, 1))
                # decoder.append(
                ...
                if self.use_lora:
                    decoder.append(
                        Layer('gen', 'lora_shrink_ff1', LayerType.FC, True, self.dtype, 
                            1, int(self.lora_rank / self.tp), self.hdim, batch))
                    decoder.append(
                        Layer('gen', 'lora_expand_ff1', LayerType.FC, True, self.dtype, 
                            1, self.hdim, int(self.lora_rank / self.tp), batch))
                ...
                if self.use_lora:
                    decoder.append(
                        Layer('gen', 'lora_shrink_ff2', LayerType.FC, True, self.dtype, 
                            1, self.hdim, int(self.lora_rank / self.tp), batch))
                    decoder.append(
                        Layer('gen', 'lora_expand_ff2', LayerType.FC, True, self.dtype, 
                            1, int(self.lora_rank / self.tp), self.hdim, batch))
